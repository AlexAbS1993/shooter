...
describe('Сущность Request организует работу запросов на удалённый сервер, нагружая тем самым базу данных', () => {
    ...
    describe('Number формируется в зависимости от названия поля', () => {
        test('Если не передано название поля, то выдается любое значение', () => {
            expect(typeof request.numberConstructor()).toBe('number')
        })
        test('Если передано название поля age, то генерируется возраст от 16 до 80', () => {
            for (let i = 0; i < 1000; i++){
                let n = request.numberConstructor('age')
                expect(n >= 16).toBe(true)
                expect(n <= 80).toBe(true)
            }
        })
        test('Если передано название поля birth_year, то генерируется год рождения', () => {
            for (let i = 0; i < 1000; i++){
                let n = request.numberConstructor('birth_year')
                expect(n >= 1930).toBe(true)
                expect(n <= 2010).toBe(true)
            }
        })
    })
})